...
| C7 | DB 스키마 지문이 검증된 목록과 불일치하면 **DB 경로를 쓰지 않고** 즉시 폴백 + 에스컬레이션 |
...
## 2'. DB → 내보내기 폴백 전환 규칙과 지문 검증 절차

1. 사이클 시작 시 `known_fingerprints.json`(검증된 스키마 지문 목록)이 비어 있으면 **kakao-db-reader를 호출하지 않고** 바로 kakao-export(폴백)로 간다.
2. A0 실행 순서: 카톡 앱 버전 확인 → DB 파일을 임시 경로로 복사(원본 미접촉) → 스키마 지문 계산·목록 대조 → 지문 일치 시에만 복호화 진행 → 대상 방 ID로 필터 + `ts > last_ts` 조회 → 결과 jsonl 저장 → **임시 복사본 즉시 삭제**.
3. **지문 불일치**는 재시도 없이 즉시 폴백 전환. 복호화·조회 오류는 1회 재시도 후 실패 시 폴백 전환.
